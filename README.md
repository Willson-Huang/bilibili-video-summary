...
> 其中**与平台无关**的部分（ASR 引擎选型、已否决的路线、校对方法论）已独立成库 👉 [**chinese-asr-notes**](https://github.com/Willson-Huang/chinese-asr-notes) —— 不做 B站 也可以直接看那边，那边更全。

**五条最省时间的**：

1. **B站 AI 字幕（`ai-zh`）是「中文 → 英文 → 中文」回译产物**——地名、人名、机构名成片偏移。实测「快递里的中国 · 广东惠州」一集里，惠州仲恺五镇被写成「铜壶铜桥莅临汇环陈江」（**一句四错**）。只有人工 CC（`zh-CN`）可信；专名密集内容一律 `--force-asr --engine funasr-nano`
2. **两个引擎的专名差距是 5 倍**：whisper-large-v3-turbo 专名 2/11，Fun-ASR-Nano 10/11；代价是慢约 4 倍（19.2x vs 5.1x 实时）。⚠️ **whisper 的错可能是高置信度错**（肇庆 → 赵庆），别只看置信度
3. **大量"看起来该有用"的路线已被真数据否决**：专名候选表覆盖 5.0%、确定性规则层真实可修 2.1%、片内一致性收益 2.3%、词表跨主题精度 0/3 —— **别重跑**，每条都附了数据
4. **校对靠「按类型核查」，不是查词表**：同一份素材包，词表法 0 个真阳性，按类型核查（企业名 → 地名 → 历史地名）3 个真错写。最危险的是**错写本身仍是合法中文词**（`姚顺雨` 被反复误识为 `尧舜禹`，一路进元数据，堵死检索入口）
5. **素材包是误识修正的唯一依据**——曾因收尾流程删掉 32 份、只能重跑一遍；现改为归档保留（`cache/bili_subs/`），且归档前有 `verify_pack` 拦截。另注意目录里存在 `sub_BV*` / `bili_BV*` 两种前缀

> 其余坑（Cookie 自动化三条路全断、元信息不能进回归基线、单测全绿 ≠ 判据正确、长任务恢复看磁盘不信汇报…）见上面两个链接。
