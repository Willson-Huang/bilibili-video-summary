...
```mermaid
flowchart LR
    A["B站链接<br/>BV / av / b23.tv"] --> B{"字幕直取？"}
    B -- "有 CC/AI 字幕" --> C["拉取字幕全文<br/>秒级"]
    B -- "无字幕 / 未登录" --> D["下载音轨 m4a<br/>不转码"]
    D --> E["本地 Whisper 转写<br/>GPU / CPU 自适应"]
    C --> F["带时间戳素材包"]
    E --> F
    F --> G["广告口播过滤<br/>70 词预标记 + AI 复核"]
    G --> H["14 节知识库条目<br/>可检索 · 可验证 · 可追溯"]
```
