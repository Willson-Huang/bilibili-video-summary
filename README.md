...
1. **B站 AI 字幕（`ai-zh`）是「中文 → 英文 → 中文」回译产物**，地名、人名、机构名成片偏移。实测「快递里的中国 · 广东惠州」一集里，惠州仲恺五镇被写成「铜壶铜桥莅临汇环陈江」，一句四错。只有人工 CC（`zh-CN`）可信，专名密集的内容一律加 `--force-asr --engine funasr-nano`
...
| 环节 | 说明 |
|---|---|
| **字幕直取** | 配置 B站 Cookie 后直接拉取字幕（秒级）；无字幕或未配置 Cookie 时自动降级为本地转写。音轨下载后不转码、转写完即删。⚠️ **「有字幕」不等于可信**：`zh-CN` 是人工 CC（可信，可作核对基准），`ai-zh` 是 B站 AI 生成，**经中文 → 英文 → 中文回译**，地名人名偏差大，**不可作验证基准**。专名密集内容建议 `--force-asr` 走本地 `funasr-nano` |
...
`ai-zh` 是机器生成，而且**经中文 → 英文 → 中文回译**：同一段口播先被识别成英文、再译回中文，地名、人名、机构名会成片偏移（同音替代叠加回译错译）。这类字幕拿来「大概了解内容」没问题，**但不能作为专名的依据，也不能作为评估其他引擎的基准**。
