...
> 完整测试设计、逐项数据与复现命令见 （含 4 项负结果的原始记录）。
...
| `--note-name <BV号>` | 只输出该 BV 的标准纪要文件名，**不改台账**；但仍需 `--token`（argparse 强制），且**依赖 `cache/bili/index.json`**——索引里没这条会报「缓存索引里没有 BVxxx，请先转写或手工补 index.json」 |
...
四个字段的值都能从素材包头部「基本信息」直接取，不用另外查接口。`transcript` **留空比填错安全**——`--finish` 的归档逻辑会依次尝试 `index[bv]['transcript']` 和默认路径 `cache/bili/bili_<BV>.md`，留空时正好落到实际文件上。补完索引，`--note-name` 与 `--finish` 都恢复正常。
...
| Nano 侧调 `batch_size` / VAD 段长 / 砍热词 / 开双进程 | **不做** | 2026-09-13 四组实测全部零收益（数字见 3b「速度」表）。Nano 是 800M 自回归解码，单进程即把 4060 Ti 吃满；速度上限由模型架构决定，不在调用参数上 |
