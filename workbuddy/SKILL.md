...
**第五个硬动作（2026-09-24 加）：中间文件的位置也要写进 prompt。** 子代理的草稿、临时脚本与临时清单只写当批的缓存目录（`.workbuddy/cache/` 下按批次建的那个目录），不得写系统临时目录、库根目录或用户主目录。此前有一批子代理在库根与工作区根各建了一个含 30 个文件的临时目录。

**派发之后主线程不要空等（2026-09-24 加）**：子代理在跑的那段时间，主线程接着做不依赖它产出的工作（写批次记忆、准备归档清单、量取文件体积）。不要等全部返回再统一复验一遍——复验并进下面的「生成后必做：结构校验」，不要为它单独设一次同步等待。
...
- 本文件只保留**当前有效的规则**与**一句实测依据**。新增的版本说明、历史沿革、旧参数取舍过程写进工作区的 `性能记录-技能沿革.md` 与 GitHub 的 Release 说明，不要再往本文件追加章节。本文件体积已在十万字节量级，靠追加维护会继续膨胀。
- 每批收尾记一次入口文件体积与技能清单元数据，写进 `性能记录-技能沿革.md` 的基线表，用来观察是否失控。
