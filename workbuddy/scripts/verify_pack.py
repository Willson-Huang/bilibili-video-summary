#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""素材包校验：确认 bili_<BV>.md 符合「可回查」要求，归档前拦截。

背景：素材包是误识修正与专名纠错的唯一依据（归档目录 cache/bili_subs/），
但此前没有任何校验——字段缺失只能靠事后统计发现。本脚本把问题拦在归档之前。
...
校验项:
  1. 标题行 + ## 基本信息 齐全（标题/BV号/UP主/发布/链接）
  2. 正文形态三选一且自洽：字幕全文 / 转写全文（正文行必须带 [hh:mm:ss] 前缀）/ 预扫描包（--only-meta，本就无正文）
  3. ## 元信息 块：8 个字段齐全、取值合法、route 与 engine 一致
  4. 确定性红线：元信息块不得含墙钟耗时或生成时间（会让包文本无法哈希回归）

宽容策略（重要）:
  默认模式把「缺元信息块」记为提醒，不判错——归档里 M1 之前的历史素材包本就没有该块；
  新产物请用 --strict（把缺块/位置异常也当错误）。校验只报告，不改文件。
...
REQUIRED_BASE = ['标题', 'BV号', 'UP主', '发布', '链接']
...
def check_pack(path):
    ...
    first = t.split('\n', 1)[0].strip()
    if first != HEAD_LINE:
        errors.append(f'首行不是素材包标题行（实际：{first[:40]}）')
    ...
    meta = _section(t, '## 元信息')
    if meta is None:
        warnings.append('缺 ## 元信息 块（M1 之前的历史产物；新产物请用 --strict 复检）')
    else:
        info['has_meta'] = True
        vals = {}
...
if __name__ == '__main__':
    log_path = Path.cwd() / 'verify_pack_out.txt'
    real = sys.stdout
