# -*- coding: utf-8 -*-
"""M9 错误基线：从归档挖掘 ASR 错误类型分布与分层纠错理论命中率。

只读脚本 —— 读 raw/ 的「ASR 误识对照表」与 cache/bili_subs/ 的素材包，
输出统计报告，不写知识库、不改任何归档。

用途：回答「M6 拼音层 / M10 分层纠错是否值得做」这个准入问题。

依赖：pypinyin（仅用于音系维度判定）。缺失时自动降级为「不做音系分类」，
      其余维度（粒度 / 特殊类 / 层①②命中）照常产出。

用法：
    # 需要 pypinyin 的 Python 环境（本机：default 或 whisper venv）
    python baseline_errors.py [--raw <raw目录>] [--subs <素材包目录>]
                              [--out <报告路径>] [--json <明细路径>]
...
DEFAULT_RAW = os.environ.get('BILI_RAW_DIR', os.path.expanduser('~/obsidian/raw'))
DEFAULT_SUBS = os.environ.get('BILI_SUBS_DIR', os.path.expanduser('~/.workbuddy/cache/bili_subs'))
